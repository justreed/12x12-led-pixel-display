...
void loop() {
  ...
  drawGame();
  FastLED.show();
  delay(150); // Control the speed of the game
}
...
void drawGame() {
  // Set the entire LED matrix to green as the background
  fill_solid(leds, NUM_LEDS, CRGB::Green);

  // Draw a yellow line in the middle
  int middleX = WIDTH / 2;
  for (int y = 0; y < HEIGHT; y++) {
    leds[XY(middleX, y)] = CRGB::Yellow;
  }

  // Draw ball
  leds[XY(ballX, ballY)] = CRGB::White;

  // Draw left paddle (three pixels wide)
  for (int i = 0; i < paddleHeight; i++) {
    for (int j = 0; j < 3; j++) {
      leds[XY(0, leftPaddleY + i + j)] = CRGB::Red;
    }
  }

  // Draw right paddle (three pixels wide)
  for (int i = 0; i < paddleHeight; i++) {
    for (int j = 0; j < 3; j++) {
      leds[XY(WIDTH - 1, rightPaddleY + i + j)] = CRGB::Blue;
    }
  }
